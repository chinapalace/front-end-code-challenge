...
import { Popup, Menu, Card, Image, Icon, Container } from "semantic-ui-react";
...
class Bodies extends Component {
  ...
  handleOnClick = name => {
    this.context.router.history.push(
      `${this.props.match.url}/${this.state.activeItem}/${name}`
    );
  };
  componentDidMount() {
    this.fetchBodies();
  ...
  fetchBodies = async () => {
    try {
      const res = await fetch(
        `/api/partners/${this.props.match.params.partner}/bodies/${
  ...
  render() {
    ...
    return (
      <Container>
        <Menu compact secondary className="filter">
          <Menu.Item
            name="female"
          ...
          <Popup
            trigger={
              <Menu.Item name="custom" active={activeItem === "custom"} />
            }
            content="Coming Soon :)"
            basic
            style={{ color: "black" }}
        ...
        <Card.Group>{this.renderData()}</Card.Group>
      </Container>
    );
  }
}
